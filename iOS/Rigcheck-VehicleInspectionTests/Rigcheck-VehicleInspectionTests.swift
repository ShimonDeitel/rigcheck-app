import XCTest
@testable import Rigcheck-VehicleInspection

@MainActor
final class Rigcheck-VehicleInspectionTests: XCTestCase {
    var store: Store!
    ...
    func testAddItemIncreasesCount() {
        let item = CheckItem(title: "Test Item")
    ...
    func testAddRespectsFreeLimit() {
        for i in 0..<Store.freeLimit {
            _ = store.add(CheckItem(title: "Item \(i)"), isPro: false)
        }
        let added = store.add(CheckItem(title: "Overflow"), isPro: false)
    ...
    func testProBypassesFreeLimit() {
        for i in 0..<Store.freeLimit {
            _ = store.add(CheckItem(title: "Item \(i)"), isPro: true)
        }
        let added = store.add(CheckItem(title: "Extra"), isPro: true)
        XCTAssertTrue(added)
        XCTAssertEqual(store.items.count, Store.freeLimit + 1)
    }

    func testDeleteByIdRemovesItem() {
        let item = CheckItem(title: "Delete Me")
        _ = store.add(item, isPro: false)
    ...
    func testUpdateChangesTitle() {
        var item = CheckItem(title: "Original")
        _ = store.add(item, isPro: false)
        item.title = "Updated"
        store.update(item)
    ...
    func testPersistenceRoundTrip() {
        let item = CheckItem(title: "Persisted")
        _ = store.add(item, isPro: false)
        let reloaded = Store()
